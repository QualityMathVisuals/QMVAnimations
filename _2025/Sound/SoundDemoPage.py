...
class StringEncirclePreview(InteractiveScene):
    def construct(self):
        ...
        self.play(t_tracker.animate.set_value(3), run_time=5, rate_func=there_and_back)
        fourier_wave.suspend_updating()
        
        # Transform to band
        self.play(self.frame.animate.set_phi(1).scale(1.2))
        self.frame.add_ambient_rotation(3 * DEGREES)
        self.play(fourier_wave.animate.rotate(90 * DEGREES, RIGHT, ORIGIN))
        self.play(fourier_wave.animate.shift(DOWN * 3))
        fourier_band = WaveFourierBand(example_func, 10, length=10, wave_config=wave_config)
        fourier_band.suspend_updating()
        self.play(ReplacementTransform(fourier_wave, fourier_band), run_time=3)
        ...
        point_fourier_wave = WaveFourierRigidBand(example_points, 35, length=10, wave_config=wave_config)
